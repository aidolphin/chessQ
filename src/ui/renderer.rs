    ...
    fn draw_board(&self, f: &mut Frame, area: Rect) {
        ...
        for rank in 0..8 {
            for file in 0..8 {
                ...
                let piece_color = if self.game_state.piece_at(square).map(|(c, _)| c)
                ...
                let paragraph =
                    Paragraph::new(Span::styled(piece_char, Style::default().fg(piece_color)))
                        .block(block)
    ...
    fn draw_sidebar(&self, f: &mut Frame, area: Rect) {
        ...
        let info_text = Text::from(vec![
            Line::from(vec![
                ...
                Span::styled(
                    if self.game_state.side_to_move == PieceColor::White {
                        "White"
                    } else {
                        "Black"
                    },
            ...
            Line::from(vec![
                ...
                Span::styled(
                    self.game_state.fullmove_number.to_string(),
                    Style::default(),
                ),
            ]),
            Line::from(vec![
        ...
        let history_text: Vec<Line> = self
            ...
            .map(|(i, mv)| {
                Line::from(vec![
                    Span::styled(
                        format!("{}. ", i / 2 + 1),
                        Style::default().fg(UiColor::DarkGray),
                    ),
    ...
    fn make_move(&mut self, mv: Move, new_state: GameState) {
        if let Some(piece) = mv.capture {
            self.captured_pieces
                .push((self.game_state.side_to_move.opposite(), piece));
        }
