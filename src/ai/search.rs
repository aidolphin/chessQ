...
impl AI {
    ...
    pub fn find_best_move(&mut self, state: &GameState, time_limit: Duration) -> SearchResult {
        ...
        // Iterative deepening
        for depth in 1..=10 {
            let (mv, eval, nodes) = self.alpha_beta(
                state,
                depth,
                -SEARCH_INF,
                SEARCH_INF,
    ...
    fn alpha_beta(
        ...
        for mv in ordered_moves.iter() {
            match GameState::make_move(state, mv) {
                Ok(new_state) => {
                    let (_, eval, nodes) = self.alpha_beta(
                        &new_state,
                        depth - 1,
                        -beta,
                        -alpha,
        ...
        self.transposition_table
            .store(hash, depth, alpha, best_move, node_type);
    ...
    fn order_moves(&self, state: &GameState, moves: Vec<Move>) -> Vec<Move> {
        let mut moves_with_score: Vec<(Move, i32)> = moves
            .into_iter()
            .map(|mv| {
