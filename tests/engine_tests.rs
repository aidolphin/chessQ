...
fn test_castling_through_check_illegal() {
    ...
    let kingside_castle = moves.iter().any(|m| m.is_castle && m.to.file() == 6);

    assert!(
        !kingside_castle,
        "Kingside castling should be illegal (castling through check)"
    );
}
...
fn test_en_passant_capture() {
    ...
    assert_eq!(
        ...
        1,
        "Should have 1 en passant capture"
    );
}
...
fn test_pinned_piece_cannot_move() {
    ...
    let rook_horizontal_moves = moves
        .iter()
        .any(|m| m.from.index() == 12 && m.to.rank() == 1 && m.to.file() != 4);

    assert!(
        !rook_horizontal_moves,
        "Pinned rook should not be able to move horizontally"
    );
}
